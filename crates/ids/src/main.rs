...
async fn main() {
    ...
    tokio::spawn(async move {
        ...
        let mut alerted_brute: std::collections::HashSet<(IpAddr, u16)> = std::collections::HashSet::new();
        loop {
            tokio::time::sleep(tokio::time::Duration::from_secs(5)).await;

            let now = Instant::now();
            let records = records_clone.lock().unwrap();
            let recent: Vec<&ConnectionRecord> = records.iter()
                .filter(|r| now.duration_since(r.timestamp).as_secs() < 30)
                .collect();
            
            let mut ip_ports: std::collections::HashMap<IpAddr, std::collections::HashSet<u16>> = std::collections::HashMap::new();
            ...
            for ((ip, port), count) in &ip_ports_count {
                if *count >= 5 && !alerted_brute.contains(&(*ip, *port)) {
                    ...
                    println!("ALERT: Brute force detected from {ip} on port {port} - {count} attempts");
                }
            }
